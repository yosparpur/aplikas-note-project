...
function local() {
    ...
    function updateNote(noteId, noteTitle, noteBody, archived) {
        const notes = loadNotesFromLocalStorage();
        const updatedNotes = notes.map(note =>
            note.id === noteId ? { ...note, title: noteTitle, body: noteBody, archived: archived } : note
        );
        localStorage.setItem('notesData', JSON.stringify(updatedNotes));
    ...
    function renderNotes(notes) {
        ...
        if (notes.length === 0) {
            ...
            notes.forEach((note) => {
                if (note.archived) return; // Jangan tampilkan catatan yang sudah diarsipkan
                ...
                editButton.onclick = () => {
                    ...
                    noteForm.querySelector('#noteArchived').checked = note.archived; // Set the archived checkbox
                    ...
                    submitButton.onclick = (event) => {
                        ...
                        updateNote(note.id,
                            ...
                            noteForm.querySelector('#noteArchived').checked // Get the archived value
                        );
                        noteForm.reset();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        noteForm.addEventListener('submit', (event) => {
            ...
            const archived = noteForm.querySelector('#noteArchived').checked; // Get the archived value
            ...
            const newNote = {
                ...
                archived: archived, // Include archived value
            };
